...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		int syllable = 0;
		List<Character> letter = Arrays.asList('a','e','i','o','u','y','A','E','I','O','U','Y');
		if (word.length()==1 && letter.contains(word.charAt(0))) {
			syllable++;
		}
		if(word.length()>1) {
			for (int i=0;i<word.length();i++) {
				if (letter.contains(word.charAt(i)) && i!=word.length()-1) {
					if (!letter.contains(word.charAt(i+1))) {
						syllable++;
					}
					else if(i+1==word.length()-1 && word.charAt(i+1)=='e') {
						syllable++;
					}
				}
				if(i==word.length()-1) {
	...
	public double getFleschScore()
	{
		...
		double x = (double) this.getNumWords() / (double) this.getNumSentences();
		double y = (double) this.getNumSyllables() / (double) this.getNumWords();
